$(document).ready(function() {
	...
	$('#subReagentPanel').each(function(){
		$(this).on('click', '.removeButton', function() {
    ...
    $(ids.dataForm).bootstrapValidator({
        ...
        fields: {
        	...
        	$.post(contextPath+"/reagent/modReagentMixture",  $form.serialize(), function(result) {
        		if(result.resultCode==0){
        			showNotice('Success',"更新混合试剂成功",'success',1000*5);
        		}else{
        			showNotice('Error','<span style="padding-top:5px">更新混合试剂失败,详情如下:</span><br/><span class="icon-exclamation-sign"><i class="glyphicon glyphicon-play"></i>'+result.resultMsg+'</span>','error',1000*10);
        		}
            },'json'); 
