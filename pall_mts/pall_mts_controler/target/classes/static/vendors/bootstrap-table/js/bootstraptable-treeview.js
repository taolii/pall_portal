...
(function ($) {
    ...
    BootstrapTable.prototype.initData = function () {
        ...
        if (that.options.treeView && this.data.length > 0) {
            ...
            var roots = $.grep(this.data, function (row, index) {
                return row[that.options.treePId]== null || row[that.options.treePId]==-1;
            });
            var g=function(child){
            ...
            $.each(roots, function (index, item) {
                ...
                if(!that.options.treeLeafField || that.options.treeLeafField==''){
                    if(child==null || child.length==0){
                        item.isLeaf=true;
                    }
                }else{
                	if(item[that.options.treeLeafField]==0){
                        item.isLeaf=false;
                    }else{
                    	item.isLeaf=true;
                    }
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        leafIcon:"glyphicon glyphicon-minus"//叶子节点样式
    });
