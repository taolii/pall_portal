$(document).ready(function() {
	...
	var _table = $table.dataTable($.extend(true,
        ...
        ajax : function(data, callback, settings) {//ajax配置为function,手动调用异步查询
            ...
            $.ajax({
                    type: "post",
                    url: contextPath+"/workflow/twiceChemicalReagentManage",
                    cache : false,  //禁用缓存
                    data: param,    //传入已封装的参数
                    dataType: "json",
	...
	$table.on("change",":checkbox",function() {
	 ...
	 var manage = {
			    ...
			    addItem: function() {
			    	LoadPage(contextPath+"/workflow/addTwiceChemicalReagent");
			    },
			    editItem: function(item) {
			    	LoadPage(contextPath+"/workflow/modTwiceChemicalReagent?crID="+item.crID+"&operator=");
			    },
