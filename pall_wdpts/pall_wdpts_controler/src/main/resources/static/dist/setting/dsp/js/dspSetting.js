$(document).ready(function() {
	...
	$table.on("change",":checkbox",function() {
	 ...
	 var manage = {
	    ...
	    fuzzySearch : true,
	    addItem: function() {
	    	LoadPage(contextPath+"/setting/addDspSetting");
	    },
	    copyItem: function(item) {
	    	LoadPage(contextPath+"/setting/modDspSetting?dsid="+item.dsid+"&operator=copy");
	    },
	    editItem: function(item) {
	    	LoadPage(contextPath+"/setting/modDspSetting?dsid="+item.dsid+"&operator=");
	    },
	    ...
	    showAssemblesDetail: function(item) {
	    	$("#dspAssembleDetailForm [name=dsid]").val(item.dsid);
	                ...
	                callback: function ($this, type) {
	                    if (type === 'yes') {
	                    	...
	                    	$(selectedItems).each(function(i) {
	                    		dsids=dsids+selectedItems[i].dsid+",";
	                        });
	                    	dsids=dsids.substr(dsids,dsids.length-1);
	                    	$.post(contextPath+"/setting/delDspSetting",{"dsids":dsids}, function(result) {
	                    		if(result.resultCode==0){
	                    			showNotice('Success',"Dsp装配配置删除成功",'success',1000*5);
	                    			$("#btn_refresh").click();
	                    		}else{
	                    			showNotice('Error','<span style="padding-top:5px">Dsp装配配置删除失败,详情如下:</span><br/><span class="icon-exclamation-sign"><i class="glyphicon glyphicon-play"></i>'+result.resultMsg+'</span>','error',1000*10);
	                    		}
	                        },'json'); 
	                    }
	                }
