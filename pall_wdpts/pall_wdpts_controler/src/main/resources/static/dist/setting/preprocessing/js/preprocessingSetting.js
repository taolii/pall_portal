$(document).ready(function() {
	...
	var _table = $table.dataTable($.extend(true,
        ...
        columns: [
            ...
            {className : "ellipsis",data: "preprocessingPn",title:"料号/PN",render : TABLE_CONSTANT.DATA_TABLES.RENDER.ELLIPSIS,width:"190px"},
            {className : "ellipsis",data: "preprocessingModel",title:"名称/Model",render : TABLE_CONSTANT.DATA_TABLES.RENDER.ELLIPSIS,width:"190px"},
            {className : "ellipsis",data: "ptOptions",title:"PT选配件/Option",render : TABLE_CONSTANT.DATA_TABLES.RENDER.ELLIPSIS,width:"190px"},
	...
	$table.on("change",":checkbox",function() {
	 ...
	 var manage = {
	    ...
	    showAssemblesDetail: function(item) {
	    	$("#preprocessingAssembleDetailForm [name=psid]").val(item.psid);
	    ...
	    showInspectsDetail: function(item) {
	    	$("#preprocessingInspectDetailForm [name=psid]").val(item.psid);
	                ...
	                callback: function ($this, type) {
	                    if (type === 'yes') {
	                    	...
	                    	$(selectedItems).each(function(i) {
	                    		psids=psids+selectedItems[i].psid+",";
	                        });
	                    	psids=psids.substr(psids,psids.length-1);
	                    	$.post(contextPath+"/setting/delPreprocessingSetting",{"psids":psids}, function(result) {
	                    		if(result.resultCode==0){
	                    			showNotice('Success',"预处理装配配置删除成功",'success',1000*5);
	                    			$("#btn_refresh").click();
	                    		}else{
	                    			showNotice('Error','<span style="padding-top:5px">预处理装配配置删除失败,详情如下:</span><br/><span class="icon-exclamation-sign"><i class="glyphicon glyphicon-play"></i>'+result.resultMsg+'</span>','error',1000*10);
	                    		}
	                        },'json'); 
	                    }
	                }
