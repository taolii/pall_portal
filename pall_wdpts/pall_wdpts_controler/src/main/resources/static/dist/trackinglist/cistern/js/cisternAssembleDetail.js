$(document).ready(function() {
	...
	var $detailTable = $("#cisternAssembleDetailTable").dataTable($.extend(true,
        ...
        ajax : function(data, callback, settings) {//ajax配置为function,手动调用异步查询
            ...
            var formData = $("#cisternAssembleDetailForm").serializeArray();
            formData.forEach(function (e) {
                param[e.name] = e.value;
            });
            $.ajax({
                    type: "post",
                    url: contextPath+"/trackinglist/cisternAssembleDetail",
                    cache : false,  //禁用缓存
                    data: param,    //传入已封装的参数
                    dataType: "json",
	...
	$("#cisternAssembleDetailButton").click(function(){
		$detailTable.draw();
	});
	$("#cisternAssembleDetailModal").on("shown.bs.modal",function(){
