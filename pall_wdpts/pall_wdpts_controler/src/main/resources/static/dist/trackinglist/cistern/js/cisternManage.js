$(document).ready(function() {
	...
	var _table = $table.dataTable($.extend(true,
        ...
        ajax : function(data, callback, settings) {//ajax配置为function,手动调用异步查询
            ...
            $.ajax({
                    type: "post",
                    url: contextPath+"/trackinglist/cisternManage",
                    cache : false,  //禁用缓存
                    data: param,    //传入已封装的参数
                    dataType: "json",
                    success: function(result) {
                        ...
                        if (result.resultCode!=0) {
                        	$(".error").html('<h3><span class="red"><i class="glyphicon glyphicon-remove"></i>水箱装配流程跟踪单查询失败,详情如下:</span><br/><span class="red icon-exclamation-sign"><i class="glyphicon glyphicon-play"></i>'+result.resultMsg+'</span>');
                    ...
                    error: function(XMLHttpRequest, textStatus, errorThrown) {
                    	...
                    	$(".error").html('<h3><span class="red"><i class="glyphicon glyphicon-remove"></i>水箱装配流程跟踪单查询失败,详情如下:</span><br/><span class="red icon-exclamation-sign"><i class="glyphicon glyphicon-play"></i>'+error+'</span>');
                        $wrapper.spinModal(false);
                    }
                });
        },
	...
	$table.on("change",":checkbox",function() {
	 ...
	 var manage = {
	    ...
	    fuzzySearch : true,
	    addItem: function() {
	    	LoadPage(contextPath+"/trackinglist/addCistern");
	    },
	    editItem: function(item) {
	    	LoadPage(contextPath+"/trackinglist/modCistern?cisternID="+item.cisternID+"&operator=");
	    },
	    copyItem: function(item) {
	    	LoadPage(contextPath+"/trackinglist/modCistern?cisternID="+item.cisternID+"&operator=copy");
	    },
	    ...
	    exportItem:function(item){
	    	$wrapper.spinModal();
	         $.post(contextPath+"/trackinglist/exportCistern",{"cisternID":item.cisternID}, function(result) {
	        	 if(result.resultCode==0){
	    ...
	    exportItems:function(){
	    	$wrapper.spinModal();
	         $.post(contextPath+"/trackinglist/exportCisterns",$queryForm.serializeArray(), function(result) {
	        	 if(result.resultCode==0){
	        		 var fileName=encodeURI(result.returnObjects[0].fileName); 
    	    		 var downUrl = contextPath+'/workflow/excelfileDownload?fileName=' +fileName+"&subDirectory="+result.returnObjects[0].subDirectory;
    	    		 window.location.href = downUrl;
	    ...
	    showAssemblesDetail: function(item) {
	    	$("#cisternAssembleDetailForm [name=cisternID]").val(item.cisternID);
	                ...
	                callback: function ($this, type) {
	                    if (type === 'yes') {
	                    	...
	                    	$(selectedItems).each(function(i) {
	                    		cisternIDs=cisternIDs+selectedItems[i].cisternID+",";
	                        });
	                    	cisternIDs=cisternIDs.substr(cisternIDs,cisternIDs.length-1);
	                    	$.post(contextPath+"/trackinglist/delCistern",{"cisternIDs":cisternIDs}, function(result) {
	                    		if(result.resultCode==0){
	                    			showNotice('Success',"水箱装配流程跟踪单删除成功",'success',1000*5);
	                    			$("#btn_refresh").click();
	                    		}else{
	                    			showNotice('Error','<span style="padding-top:5px">水箱装配流程跟踪单删除失败,详情如下:</span><br/><span class="icon-exclamation-sign"><i class="glyphicon glyphicon-play"></i>'+result.resultMsg+'</span>','error',1000*10);
	                    		}
	                        },'json'); 
	                    }
	                }
