$(document).ready(function() {
	...
	var $detailTable = $("#dspAssembleDetailTable").dataTable($.extend(true,
        ...
        ajax : function(data, callback, settings) {//ajax配置为function,手动调用异步查询
            ...
            var formData = $("#dspAssembleDetailForm").serializeArray();
            formData.forEach(function (e) {
                param[e.name] = e.value;
            });
            $.ajax({
                    type: "post",
                    url: contextPath+"/trackinglist/dspAssembleDetail",
                    cache : false,  //禁用缓存
                    data: param,    //传入已封装的参数
                    dataType: "json",
	...
	$("#dspAssembleDetailButton").click(function(){
		$detailTable.draw();
	});
	$("#dspAssembleDetailModal").on("shown.bs.modal",function(){
