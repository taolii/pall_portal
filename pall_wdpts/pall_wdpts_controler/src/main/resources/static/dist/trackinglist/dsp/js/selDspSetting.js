var DspSettingModal=function(contextPath,ids){
	var dspSettingModal=new Object();
	dspSettingModal.contextPath=contextPath;
	dspSettingModal.ids=ids;
	dspSettingModal.initModalTable=function(){
		var $wrapper = $(ids.wrapper);
		var $table = $(ids.dspSettingDatatable);
		...
		var datatables = $table.dataTable($.extend(true,
	        ...
	        ajax : function(data, callback, settings) {//ajax配置为function,手动调用异步查询
	            ...
	            $.ajax({
	                    type: "post",
	                    url: contextPath+"/setting/dspSetting",
	                    cache : false,  //禁用缓存
	                    data: param,    //传入已封装的参数
	                    dataType: "json",
	                    success: function(result) {
	                        ...
	                        if (result.resultCode!=0) {
	                        	$(".error").html('<h3><span class="red"><i class="glyphicon glyphicon-remove"></i>Dsp装配配置查询失败,详情如下:</span><br/><span class="red icon-exclamation-sign"><i class="glyphicon glyphicon-play"></i>'+result.resultMsg+'</span>');
	                    ...
	                    error: function(XMLHttpRequest, textStatus, errorThrown) {
	                    	...
	                    	$(".error").html('<h3><span class="red"><i class="glyphicon glyphicon-remove"></i>Dsp装配配置查询失败,详情如下:</span><br/><span class="red icon-exclamation-sign"><i class="glyphicon glyphicon-play"></i>'+error+'</span>');
	                        $wrapper.spinModal(false);
	                    }
	                });
	        },
	        columns: [
	            ...
	            {className : "ellipsis",data: "dspPn",title:"料号/PN",render : TABLE_CONSTANT.DATA_TABLES.RENDER.ELLIPSIS,width:"220px"},
		...
		$(ids.dspSettingDatatable+"_wrapper").find(".dataTables_length").parent().parent().hide();
		$(ids.dspSettingDatatable).on("click",ids.assemblesDetail,function() {
	    	var item = _table.row($(this).closest('tr')).data();
	    	$(ids.queryForm+" [name=dsid]").val(item.dsid);
	    	$dspAssembleDetailModal=$(ids.modal);
	...
	dspSettingModal.initModalShut=function(modalDataTables,dspAssembleTable,dspAssembleDatatables){
		$(ids.shutModalButton).click(function(){
			var radios=$("tbody :radio:checked",$(ids.dspSettingDatatable));
			if(radios.length<=0){
				showNotice('Error','请选择一条记录','error',1000*2);
			}else{
				$(radios).each(function(i) {
					var item = modalDataTables.row($(this).closest('tr')).data();
					$(ids.dataForm+" [name=dsid]").val(item.dsid);
